...
(function(global) {
    ...
    function processFooWidget(fooWidget) {
        ...
        createFooWidget(fooWidget,path);
    }

    function createFooWidget(fooWidget,path) {
            <% if (useIframe) { %>
                var iframe = document.createElement('iframe');
            iframe.setAttribute('src', serverHost+'/api/3rd/foo/widget/'+path+'/init?iframe=true&partyId='+partyId);
