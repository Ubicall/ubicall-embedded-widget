...
function createCall($, pageId, queue, title) {
    ...
    var $buttonb = $("<button/>").attr("class", "btn btn-default").text("Receive a call on Cell phone")
        .attr("onclick", "this.disabled=true;UbiCallManager.setPhoneCallQueue(" + queue + ");UbiCallManager.goTosubmitPhoneCall();this.disabled=false;UbiCallManager.getWorkingHours(" + queue + ")");
